...
class TuntasHub
  ...
  def self.pricing_plan
    return 'community' unless TuntasApp.enterprise?

    InstallationConfig.find_by(name: 'INSTALLATION_PRICING_PLAN')&.value || 'community'
  end

  def self.pricing_plan_quantity
    return 0 unless TuntasApp.enterprise?
  ...
  def self.support_config
    {
      support_website_token: InstallationConfig.find_by(name: 'TUNTAS_SUPPORT_WEBSITE_TOKEN')&.value,
      support_script_url: InstallationConfig.find_by(name: 'TUNTAS_SUPPORT_SCRIPT_URL')&.value,
      support_identifier_hash: InstallationConfig.find_by(name: 'TUNTAS_SUPPORT_IDENTIFIER_HASH')&.value
    }
  end
  ...
  def self.instance_config
    {
      ...
      installation_version: Tuntas.config[:version],
  ...
  def self.sync_with_hub
    return unless ENV['ENABLE_HUB_TELEMETRY']
...
TuntasHub.singleton_class.prepend_mod_with('TuntasHub')
