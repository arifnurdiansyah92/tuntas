class Webhooks::Trigger
  ...
  def request_headers(body)
    ...
    headers['X-Tuntas-Delivery'] = @delivery_id if @delivery_id.present?
    if @secret.present?
      ts = Time.now.to_i.to_s
      headers['X-Tuntas-Timestamp'] = ts
      headers['X-Tuntas-Signature'] = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', @secret, "#{ts}.#{body}")}"
    end
    headers
  end
