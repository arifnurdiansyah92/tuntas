...
<template>
  ...
  >
    ...
    <p class="text-sm font-normal text-n-slate-11">
      ...
      <span v-if="isOnTuntasCloud && !isAdmin">
        {{ $t('GENERAL_SETTINGS.LIMIT_MESSAGES.NON_ADMIN') }}
      </span>
      <span v-else-if="!isOnTuntasCloud && !isSuperAdmin">
        {{ $t(`${featurePrefix}.ENTERPRISE_PAYWALL.ASK_ADMIN`) }}
      </span>
    </p>
    <template v-if="isOnTuntasCloud && isAdmin">
    ...
    <template v-else-if="!isOnTuntasCloud && isSuperAdmin">
