...
const fetchSSLStatus = () => {
  if (!isOnTuntasCloud.value) return;
...
const handleUpdatePortalConfiguration = portalObj => {
  ...
  // If custom domain is added or updated, fetch SSL status after a delay of 5 seconds (only on Tuntas cloud)
  if (portalObj?.custom_domain && isOnTuntasCloud.value) {
    setTimeout(() => {
      fetchSSLStatus();
