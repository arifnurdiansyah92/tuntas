...
export function useBranding() {
  ...
  /**
   * Replaces "Tuntas" (any casing) in text with the installation name from
   * global config
   * @param {string} text - The text to process
   * @returns {string} - Text with "Tuntas" replaced by installation name
   */
  const replaceInstallationName = text => {
    ...
    return text.replace(/tuntas/gi, installationName);
  };
