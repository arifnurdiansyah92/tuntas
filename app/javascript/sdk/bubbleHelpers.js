...
import { TUNTAS_CLOSED, TUNTAS_OPENED } from '../widget/constants/sdkEvents';
...
export const setBubbleText = bubbleText => {
  if (isExpandedView(window.$tuntas.type)) {
...
export const createBubbleIcon = ({ className, path, target }) => {
  let bubbleClassName = `${className} woot-elements--${window.$tuntas.position}`;
...
const handleBubbleToggle = newIsOpen => {
  ...
  if (newIsOpen) {
    dispatchWindowEvent({ eventName: TUNTAS_OPENED });
  } else {
    dispatchWindowEvent({ eventName: TUNTAS_CLOSED });
    chatBubble.focus();
  }
};
...
export const onBubbleClick = (props = {}) => {
  ...
  const { isOpen } = window.$tuntas;
  if (isOpen === toggleValue) return;

  const newIsOpen = toggleValue === undefined ? !isOpen : toggleValue;
  window.$tuntas.isOpen = newIsOpen;
