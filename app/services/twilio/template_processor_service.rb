class Twilio::TemplateProcessorService
  ...
  def convert_text_template(tuntas_params)
    return process_key_value_params(tuntas_params['processed_params']) if tuntas_params['processed_params'].present?

    process_whatsapp_format_params(tuntas_params['parameters'])
  end
  ...
  def convert_media_template(tuntas_params)
    content_variables = {}

    # Handle processed_params format (key-value pairs)
    if tuntas_params['processed_params'].present?
      tuntas_params['processed_params'].each do |key, value|
        content_variables[key.to_s] = value.to_s
      end
    else
      # Handle parameters format (WhatsApp Cloud API format)
      parameter_index = 1
      tuntas_params['parameters']&.each do |component|
