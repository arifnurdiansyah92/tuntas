...
class Telegram::IncomingMessageService
  ...
  def perform
    # tuntas doesn't support group conversations at the moment
    ...
    # Tuntas defines a 'read' status in its enum but does not currently update this status for Telegram conversations.
    # We have two options:
    # 1. Send the read request to Telegram here, immediately when the message is created.
    # 2. Properly update the read status in the Tuntas UI and trigger the Telegram request when the agent actually reads the message.
