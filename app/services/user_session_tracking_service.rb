class UserSessionTrackingService
  ...
  LEGACY_MOBILE_UAS = [
    ...
    { match: %r{\ATuntas/.*CFNetwork.*Darwin}, platform: 'iPhone', device: 'iPhone' }
  ].freeze
  ...
  def mobile_client_headers
    name = @request.headers['X-Tuntas-Client-Name']
    return nil if name.blank?

    platform = @request.headers['X-Tuntas-Platform']
    model = @request.headers['X-Tuntas-Device-Model']

    {
      browser_name: name,
      browser_version: @request.headers['X-Tuntas-Client-Version'],
      device_name: device_name_for_icon(platform, model),
      platform_name: model,
      platform_version: @request.headers['X-Tuntas-Platform-Version']
    }
  end
