...
#  tuntas_record_type :string           not null
...
#  tuntas_record_id   :bigint           not null
...
#  idx_data_import_mappings_on_record              (tuntas_record_type,tuntas_record_id)
...
class DataImportMapping < ApplicationRecord
  ...
  validates :source_provider, :source_object_type, :source_object_id, :tuntas_record_type, :tuntas_record_id, presence: true
  validates :source_object_id, uniqueness: { scope: [:account_id, :source_provider, :source_object_type] }

  def tuntas_record
    tuntas_record_type.constantize.find_by(id: tuntas_record_id)
  end
end
