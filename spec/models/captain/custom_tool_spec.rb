  ...
  describe 'Toolable concern' do
    ...
    describe '#build_request_body' do
      ...
      it 'renders request body template with params' do
        tool = create(:captain_custom_tool, account: account,
                                            request_template: '{ "order_id": "{{ order_id }}", "source": "tuntas" }')

        result = tool.build_request_body({ order_id: '12345' })
        expect(result).to eq('{ "order_id": "12345", "source": "tuntas" }')
      end
    end
    ...
    describe '#build_metadata_headers' do
      ...
      it 'includes account and assistant metadata' do
        ...
        expect(headers['X-Tuntas-Account-Id']).to eq(account.id.to_s)
        expect(headers['X-Tuntas-Assistant-Id']).to eq('123')
      end

      it 'includes tool slug' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Tuntas-Tool-Slug']).to eq('custom_test_tool')
      end

      it 'includes conversation metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Tuntas-Conversation-Id']).to eq(conversation.id.to_s)
        expect(headers['X-Tuntas-Conversation-Display-Id']).to eq(conversation.display_id.to_s)
      end

      it 'includes contact metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Tuntas-Contact-Id']).to eq(contact.id.to_s)
        expect(headers['X-Tuntas-Contact-Email']).to eq(contact.email)
      end

      it 'includes contact inbox verification metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Tuntas-Contact-Inbox-Id']).to eq(conversation.contact_inbox.id.to_s)
        expect(headers['X-Tuntas-Contact-Inbox-Verified']).to eq(conversation.contact_inbox.hmac_verified.to_s)
      end
      ...
      it 'handles missing conversation gracefully' do
        ...
        expect(headers['X-Tuntas-Conversation-Id']).to be_nil
        expect(headers['X-Tuntas-Conversation-Display-Id']).to be_nil
        expect(headers['X-Tuntas-Account-Id']).to eq(account.id.to_s)
      end
      ...
      it 'handles missing contact gracefully' do
        ...
        expect(headers['X-Tuntas-Contact-Id']).to be_nil
        expect(headers['X-Tuntas-Contact-Email']).to be_nil
        expect(headers['X-Tuntas-Account-Id']).to eq(account.id.to_s)
      end
      ...
      it 'handles missing contact inbox gracefully' do
        ...
        expect(headers['X-Tuntas-Contact-Inbox-Id']).to be_nil
        expect(headers['X-Tuntas-Contact-Inbox-Verified']).to eq('false')
      end
      ...
      it 'omits contact email header when email is blank' do
        ...
        expect(headers).not_to have_key('X-Tuntas-Contact-Email')
      end
      ...
      it 'omits contact phone header when phone number is blank' do
        ...
        expect(headers).not_to have_key('X-Tuntas-Contact-Phone')
      end
