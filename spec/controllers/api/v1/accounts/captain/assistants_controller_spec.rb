  ...
  describe 'POST /api/v1/accounts/{account.id}/captain/assistants' do
    ...
    context 'when it is an admin' do
      it 'creates a new assistant' do
        ...
        expect(json_response[:config][:product_name]).to eq('Tuntas')
  ...
  describe 'PATCH /api/v1/accounts/{account.id}/captain/assistants/{id}' do
    ...
    context 'when it is an admin' do
      ...
      it 'updates auto_resolve_mode without replacing other config' do
        assistant.update!(config: { 'product_name' => 'Tuntas', 'auto_resolve_mode' => 'legacy' })
        ...
        expect(response).to have_http_status(:success)
        expect(assistant.reload.config).to include('product_name' => 'Tuntas', 'auto_resolve_mode' => 'disabled')
      end
