...
describe Messages::Instagram::Messenger::MessageBuilder do
  ...
  let!(:instagram_messenger_channel) { create(:channel_instagram_fb_page, account: account, instagram_id: 'tuntas-app-user-id-1') }
  ...
  describe '#perform' do
      ...
      allow(fb_object).to receive(:get_object).and_return(
        {
          ...
          profile_pic: 'https://tuntas-assets.local/sample.png'
    ...
    it 'discard echo message already sent by tuntas' do
      messaging = dm_params[:entry][0]['messaging'][0]
      sender_id = messaging['sender']['id']
    ...
    it 'creates message with for reply with story id' do
      ...
      expect(message.content_attributes[:story_id]).to eq('tuntas-app-user-id-1')
