...
describe Messages::Facebook::MessageBuilder do
  ...
  describe '#perform' do
    ...
    it 'raises exception for non profile account' do
      ...
      contact = facebook_channel.inbox.contacts.first
      # Refer: https://github.com/chatwoot/tuntas/pull/3016 for this check
    ...
    context 'when message contains a reel attachment' do
      ...
      before do
        allow(Koala::Facebook::API).to receive(:new).and_return(fb_object)
        allow(fb_object).to receive(:get_object).and_return(
          { first_name: 'Jane', last_name: 'Dae', profile_pic: 'https://tuntas-assets.local/sample.png' }.with_indifferent_access
        )
      end
