...
describe Webhooks::Trigger do
  ...
  describe 'request headers' do
    ...
    context 'with delivery_id' do
      it 'adds X-Tuntas-Delivery header' do
        expect(SafeFetch).to receive(:fetch) do |received_url, **options, &block|
          expect(received_url).to eq(url)
          expect(options[:headers]['X-Tuntas-Delivery']).to eq('test-uuid')
          expect(options[:headers]).not_to have_key('X-Tuntas-Signature')
          expect(options[:headers]).not_to have_key('X-Tuntas-Timestamp')
    ...
    context 'with secret' do
      ...
      it 'adds X-Tuntas-Timestamp header' do
        expect(SafeFetch).to receive(:fetch) do |_received_url, **options, &block|
          expect(options[:headers]['X-Tuntas-Timestamp']).to match(/\A\d+\z/)
          block.call(fetch_result)
        end
      ...
      it 'adds X-Tuntas-Signature header with correct HMAC' do
        expect(SafeFetch).to receive(:fetch) do |_received_url, **options, &block|
          ts = options[:headers]['X-Tuntas-Timestamp']
          expected_sig = "sha256=#{OpenSSL::HMAC.hexdigest('SHA256', secret, "#{ts}.#{body}")}"
          expect(options[:headers]['X-Tuntas-Signature']).to eq(expected_sig)
          block.call(fetch_result)
        end
      ...
      it 'signs timestamp.body not just body' do
        expect(SafeFetch).to receive(:fetch) do |_received_url, **options, &block|
          ...
          expect(options[:headers]['X-Tuntas-Signature']).not_to eq(wrong_sig)
          block.call(fetch_result)
        end
    ...
    context 'with both secret and delivery_id' do
      it 'includes all three security headers' do
        expect(SafeFetch).to receive(:fetch) do |_received_url, **options, &block|
          expect(options[:headers]['X-Tuntas-Delivery']).to eq('abc-123')
          expect(options[:headers]['X-Tuntas-Timestamp']).to be_present
          expect(options[:headers]['X-Tuntas-Signature']).to start_with('sha256=')
          block.call(fetch_result)
        end
