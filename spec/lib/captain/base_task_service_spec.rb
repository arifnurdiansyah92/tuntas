...
RSpec.describe Captain::BaseTaskService do
  ...
  describe 'error handling' do
    ...
    let(:exception_tracker) { instance_double(TuntasExceptionTracker) }

    before do
      allow(Llm::Config).to receive(:with_api_key).and_raise(error)
      allow(TuntasExceptionTracker).to receive(:new).with(error, account: account).and_return(exception_tracker)
      allow(exception_tracker).to receive(:capture_exception)
    end

    it 'tracks exceptions' do
      expect(TuntasExceptionTracker).to receive(:new).with(error, account: account).and_return(exception_tracker)
