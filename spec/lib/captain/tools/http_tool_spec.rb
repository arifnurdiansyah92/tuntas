    ...
    context 'with metadata headers' do
      ...
      it 'includes metadata headers in GET request' do
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-Tuntas-Account-Id' => account.id.to_s,
                  'X-Tuntas-Assistant-Id' => assistant.id.to_s,
                  'X-Tuntas-Tool-Slug' => custom_tool.slug,
                  'X-Tuntas-Conversation-Id' => conversation.id.to_s,
                  'X-Tuntas-Conversation-Display-Id' => conversation.display_id.to_s,
                  'X-Tuntas-Contact-Inbox-Id' => conversation.contact_inbox.id.to_s,
                  'X-Tuntas-Contact-Inbox-Verified' => conversation.contact_inbox.hmac_verified.to_s,
                  'X-Tuntas-Contact-Id' => contact.id.to_s,
                  'X-Tuntas-Contact-Email' => contact.email
                })
      ...
      it 'defaults contact inbox verified header to false when contact inbox is missing' do
        ...
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-Tuntas-Contact-Inbox-Verified' => 'false'
                })
          .to_return(status: 200, body: '{"success": true}')
        ...
        expect(WebMock).to have_requested(:get, 'https://example.com/api/data')
          .with(headers: { 'X-Tuntas-Contact-Inbox-Verified' => 'false' })
      end
      ...
      it 'includes contact phone when present' do
        ...
        stub_request(:get, 'https://example.com/api/data')
          .with(headers: {
                  'X-Tuntas-Contact-Phone' => '+1234567890'
                })
          .to_return(status: 200, body: '{"success": true}')
        ...
        expect(WebMock).to have_requested(:get, 'https://example.com/api/data')
          .with(headers: { 'X-Tuntas-Contact-Phone' => '+1234567890' })
      end
