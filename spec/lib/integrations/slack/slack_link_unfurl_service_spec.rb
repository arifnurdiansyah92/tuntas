      ...
      let(:link_shared) do
        {
          ...
          api_app_id: 'A012S5UETV4',
          event: link_shared_event.merge(links: [{
                                           url: "https://qa.tuntas.id/app/accounts/1212/conversations/#{conversation.display_id}",
                                           domain: 'qa.tuntas.id'
                                         }], channel: 'G054F6A6Q'),
          type: 'event_callback',
          event_time: 1_588_623_033
      ...
      let(:link_shared) do
        {
          ...
          api_app_id: 'A012S5UETV4',
          event: link_shared_event.merge(links: [{
                                           url: 'https://qa.tuntas.id/app/accounts/1/conversations/1213',
                                           domain: 'qa.tuntas.id'
                                         }], channel: 'G054F6A6Q'),
          type: 'event_callback',
          event_time: 1_588_623_033
    ...
    context 'when the event contains containing single link' do
      let(:link_shared) do
        {
          ...
          api_app_id: 'A012S5UETV4',
          event: link_shared_event.merge(links: [{
                                           url: "https://qa.tuntas.id/app/accounts/1/conversations/#{conversation.display_id}",
                                           domain: 'qa.tuntas.id'
                                         }]),
          type: 'event_callback',
          event_time: 1_588_623_033
      ...
      it 'sends a POST unfurl request to Slack' do
        expected_body = {
          ...
          'unfurls' => '{"https://qa.tuntas.id/app/accounts/1/conversations/1":' \
                       '{"blocks":[{' \
                       '"type":"section",' \
                       '"fields":[{' \
                       '"type":"mrkdwn",' \
                       ...
                       '"url":"https://qa.tuntas.id/app/accounts/1/conversations/1",' \
        ...
        result = link_builder.perform
        expect(result).to eq([{ url: 'https://qa.tuntas.id/app/accounts/1/conversations/1', domain: 'qa.tuntas.id' }])
      end
    end
