...
RSpec.describe ConversationReplyMailer do
  describe 'reply' do
      ...
      context 'when message is a CSAT survey' do
        ...
        it 'includes CSAT survey URL in outgoing_content' do
          with_modified_env 'FRONTEND_URL' => 'https://app.tuntas.id' do
            ...
            expect(mail.decoded).to include "https://app.tuntas.id/survey/responses/#{conversation.uuid}"
          end
        end
    ...
    context 'when inbox email address is available' do
      let(:inbox) { create(:inbox, account: account, email_address: 'noreply@tuntas.id') }
