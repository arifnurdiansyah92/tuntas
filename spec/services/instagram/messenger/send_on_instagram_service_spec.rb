...
describe Instagram::Messenger::SendOnInstagramService do
  ...
  let!(:instagram_channel) { create(:channel_instagram_fb_page, account: account, instagram_id: 'tuntas-app-user-id-1') }
  ...
  describe '#perform' do
    context 'with reply' do
      ...
      context 'without message_tag HUMAN_AGENT' do
        ...
        it 'if message is sent from tuntas and is outgoing' do
        ...
        it 'if message is sent from tuntas and is outgoing with multiple attachments' do
          message = build(
            :message,
            content: nil,
        ...
        it 'if message with attachment is sent from tuntas and is outgoing' do
        ...
        it 'if message sent from tuntas is failed' do
          message = create(:message, message_type: 'outgoing', inbox: instagram_messenger_inbox, account: account, conversation: conversation)
