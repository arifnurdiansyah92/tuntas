...
RSpec.describe UserSessionTrackingService do
  ...
  describe '#create_or_update!' do
    ...
    context 'with a Tuntas Mobile legacy User-Agent' do
      context 'when the UA is okhttp (Android Tuntas Mobile)' do
        let(:ua) { 'okhttp/4.9.2' }

        it 'labels the session as Tuntas Mobile on Android', :aggregate_failures do
          service.create_or_update!

          session = user.user_sessions.last
          expect(session.browser_name).to eq('Tuntas Mobile')
      ...
      context 'when the UA is CFNetwork (iOS Tuntas Mobile)' do
        let(:ua) { 'Tuntas/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

        it 'labels the session as Tuntas Mobile on iPhone', :aggregate_failures do
          service.create_or_update!
    ...
    context 'with X-Tuntas-* structured headers' do
      let(:ua) { 'Tuntas/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

      context 'when platform is ios and model is an iPhone' do
        let(:headers) do
          {
            'X-Tuntas-Client-Name' => 'Tuntas Mobile',
            'X-Tuntas-Client-Version' => '4.7.0',
            'X-Tuntas-Platform' => 'ios',
            'X-Tuntas-Platform-Version' => '18.2',
            'X-Tuntas-Device-Model' => 'iPhone 15 Pro'
          }
        end
        ...
        let(:headers) do
          {
            ...
            'X-Tuntas-Platform' => 'ios',
            'X-Tuntas-Platform-Version' => '18.2',
            'X-Tuntas-Device-Model' => 'iPad Pro 11-inch'
          }
        end
        ...
        let(:headers) do
          {
            'X-Tuntas-Client-Name' => 'Tuntas Mobile',
            'X-Tuntas-Client-Version' => '4.7.0',
            'X-Tuntas-Platform' => 'android',
            'X-Tuntas-Platform-Version' => '14',
            'X-Tuntas-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when X-Tuntas-Client-Name is blank' do
        let(:ua) { 'okhttp/4.9.2' }
        let(:headers) do
          {
            'X-Tuntas-Client-Name' => '',
            'X-Tuntas-Platform' => 'android',
            'X-Tuntas-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when no X-Tuntas-* headers are sent (real browser)' do
        let(:ua) { 'Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 Firefox/124.0' }
        let(:headers) { {} }
