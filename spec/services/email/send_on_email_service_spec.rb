...
describe Email::SendOnEmailService do
  ...
  describe '#perform' do
    ...
    context 'when an error occurs' do
      ...
      let(:exception_tracker) { instance_double(TuntasExceptionTracker, capture_exception: true) }
      ...
      before do
        ...
        allow(TuntasExceptionTracker).to receive(:new).and_return(exception_tracker)
      end

      it 'captures the exception' do
        expect(TuntasExceptionTracker).to receive(:new).with(error, account: message.account)
