...
# Description: Install and manage a Tuntas installation.
...
TUNTAS_HUB_URL="https://hub.2.chatwoot.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/tuntas-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Tuntas (e.g., tuntas.domain.com): ' domain_name
...
serve Tuntas at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "tuntas"; then
    adduser --disabled-password --gecos "" tuntas
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/tuntas/config
  file="/opt/tuntas/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/tuntas/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create tuntas db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER tuntas CREATEDB;
    ALTER USER tuntas PASSWORD :'pass';
    ALTER ROLE tuntas SUPERUSER;
...
function setup_tuntas() {
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=tuntas/' .env
...
function run_db_migrations(){
  sudo -i -u tuntas << EOF
  cd tuntas
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:tuntas_prepare
EOF
}

##############################################################################
# Setup Tuntas systemd services and cwctl CLI
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  if [ -f "/etc/systemd/system/tuntas.target" ]; then
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop tuntas.target || true
      systemctl disable tuntas.target || true
      systemctl stop tuntas-worker.1.service || true
      systemctl disable tuntas-worker.1.service || true
    fi

    # Stop and disable worker target if converting from worker-only
    if [ -f "/etc/systemd/system/tuntas-worker.target" ]; then
      echo "Converting from worker-only to web-only"
      systemctl stop tuntas-worker.target || true
      systemctl disable tuntas-worker.target || true
    fi

    cp /home/tuntas/tuntas/deployment/tuntas-web.1.service /etc/systemd/system/tuntas-web.1.service
    cp /home/tuntas/tuntas/deployment/tuntas-web.target /etc/systemd/system/tuntas-web.target

    systemctl daemon-reload
    systemctl enable tuntas-web.target
    systemctl start tuntas-web.target
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop tuntas.target || true
      systemctl disable tuntas.target || true
      systemctl stop tuntas-web.1.service || true
      systemctl disable tuntas-web.1.service || true
    fi

    # Stop and disable web target if converting from web-only
    if [ -f "/etc/systemd/system/tuntas-web.target" ]; then
      echo "Converting from web-only to worker-only"
      systemctl stop tuntas-web.target || true
      systemctl disable tuntas-web.target || true
    fi

    cp /home/tuntas/tuntas/deployment/tuntas-worker.1.service /etc/systemd/system/tuntas-worker.1.service
    cp /home/tuntas/tuntas/deployment/tuntas-worker.target /etc/systemd/system/tuntas-worker.target

    systemctl daemon-reload
    systemctl enable tuntas-worker.target
    systemctl start tuntas-worker.target
    ...
    cp /home/tuntas/tuntas/deployment/tuntas-web.1.service /etc/systemd/system/tuntas-web.1.service
    cp /home/tuntas/tuntas/deployment/tuntas-worker.1.service /etc/systemd/system/tuntas-worker.1.service
    cp /home/tuntas/tuntas/deployment/tuntas.target /etc/systemd/system/tuntas.target

    systemctl daemon-reload
    systemctl enable tuntas.target
    systemctl start tuntas.target
  fi

  cp /home/tuntas/tuntas/deployment/tuntas /etc/sudoers.d/tuntas
  cp /home/tuntas/tuntas/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/tuntas/tuntas/develop/deployment/nginx_tuntas.conf
  cp nginx_tuntas.conf /etc/nginx/sites-available/nginx_tuntas.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/tuntas.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_tuntas.conf
  ln -s /etc/nginx/sites-available/nginx_tuntas.conf /etc/nginx/sites-enabled/nginx_tuntas.conf
...
EOF
  ...
  # Restart the appropriate tuntas target
  if [ -f "/etc/systemd/system/tuntas-web.target" ]; then
    systemctl restart tuntas-web.target
  elif [ -f "/etc/systemd/system/tuntas-worker.target" ]; then
    systemctl restart tuntas-worker.target
  else
    systemctl restart tuntas.target
  fi
}
...
function setup_logging() {
  touch /var/log/tuntas-setup.log
  LOG_FILE="/var/log/tuntas-setup.log"
}
...
***************************************************************************
Woot! Woot!! Tuntas server installation is complete.
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Tuntas CLI tool to manage your installation.'
...
***************************************************************************
              Tuntas Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/tuntas-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Tuntas?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Tuntas. This takes a long while."
  setup_tuntas &>> "${LOG_FILE}"
...
https://www.chatwoot.com/docs/deployment/deploy-tuntas-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u tuntas bash -c " cd tuntas && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Tuntas installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Tuntas
  -I BRANCH                 Install Tuntas from a git branch
  -u, --upgrade             Upgrade Tuntas to the latest stable version
  -U BRANCH                 Upgrade Tuntas from a git branch (EXPERIMENTAL)
...
Management:
  ...
  -l, --logs                View logs from Tuntas. Supported values include web/worker.
  -r, --restart             Restart Tuntas server
...
##############################################################################
# Get Tuntas web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u tuntas-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u tuntas-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Tuntas v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Tuntas 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Tuntas to v$CW_VERSION (branch: $BRANCH)"
...
EOF
  ...
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Tuntas v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the Tuntas directory
  cd tuntas
...
##############################################################################
# Restart Tuntas server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  if [ -f "/etc/systemd/system/tuntas-web.target" ]; then
    systemctl restart tuntas-web.target
    systemctl status tuntas-web.target
  elif [ -f "/etc/systemd/system/tuntas-worker.target" ]; then
    systemctl restart tuntas-worker.target
    systemctl status tuntas-worker.target
  else
    systemctl restart tuntas.target
    systemctl status tuntas.target
  fi
}

##############################################################################
# Convert existing Tuntas deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting Tuntas deployment to: $DEPLOYMENT_TYPE"

  # Check if Tuntas is installed
  if [ ! -d "/home/tuntas/tuntas" ]; then
    echo "Tuntas installation not found. Use --install first."
    exit 1
  fi
...
function report_event() {
  ...
  curl -X POST -H "Content-Type: application/json" -d "$data" "$TUNTAS_HUB_URL" -s -o /dev/null
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u tuntas << "EOF"
  cd tuntas
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/tuntas/tuntas/master/VERSION_CWCTL"
